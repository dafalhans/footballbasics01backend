    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http.csrf().disable()
                ...
                .authorizeHttpRequests(auth ->
                    auth.
                            requestMatchers(HttpMethod.GET,"/api/teams").permitAll().    // alle GETs van reguliere DTOs mogen door...
                            requestMatchers(HttpMethod.GET,"/api/teams/**").permitAll().
                            requestMatchers(HttpMethod.GET,"/api/players").permitAll().
                            requestMatchers(HttpMethod.GET,"/api/players/**").permitAll().
                            requestMatchers(HttpMethod.GET,"/api/trainers").permitAll().
                            requestMatchers(HttpMethod.GET,"/api/trainers/**").permitAll().
                            requestMatchers(HttpMethod.GET,"/api/enums").permitAll().    // enums van TrainerSpecialty en Player Basis Opzet positie
                            requestMatchers(HttpMethod.GET,"/api/enums/**").permitAll(). // enums van TrainerSpecialty en Player Basis Opzet positie
                            ...
                            requestMatchers(HttpMethod.GET,"/explorer/**").permitAll().
                            anyRequest().authenticated()).oauth2Login(withDefaults()).     // voor de rest willen we een JWT (verwachting dat het uit de Front End komt)
                    oauth2ResourceServer(oauth2 -> oauth2.jwt(jwt -> jwt.jwtAuthenticationConverter(demoConvertor())));
//                .authorizeHttpRequests(auth ->
